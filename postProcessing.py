...
for j in  range(0,it-1):
	...
	case = os.path.join('/home/giacomol/Desktop/Research/windLoading/LES/optimization/highRiseDomain/XCDF' +str(j)) 
...
fig = '/home/giacomol/Desktop/Research/slides/progress/figures'
...
plt.plot(U_exp, zsample, '-.', color='gray')	
plt.fill_betweenx(zsample, U_exp_min, U_exp_max, color='silver')
plt.plot(U[:,0], zprobes, '--', color='black', linewidth=2.0)
#plt.plot(U[:,1:]/ustar, zprobes)
...
plt.ylabel(r'$y [m]$')
plt.legend(['exp','base'])
plt.savefig(os.path.join(fig, 'U_base.eps'))
plt.show()

plt.figure(figsize=(6,4))
matplotlib.rcParams.update({'font.size': 20})
plt.plot(k_exp, zsample, '-', color='black')
plt.plot(k_exp_min, zsample, '-.', color='black')
plt.plot(k_exp_max, zsample, '-.', color='black')
plt.fill_betweenx(zsample, k_exp_min, k_exp_max, color='silver')
#plt.plot(k, zprobes)
plt.gcf().subplots_adjust(bottom=0.2)
...
plt.savefig(os.path.join(fig, 'TKE_UQ.eps'))
plt.show()

'''
matplotlib.rcParams.update({'font.size': 20})
plt.plot(uu_exp, zsample, '-.', color='gray')	
plt.fill_betweenx(zsample, uu_exp_min, uu_exp_max, color='silver')
plt.plot(uu, zprobes)
plt.gcf().subplots_adjust(bottom=0.2)
plt.xlabel(r"$\overline{u'^2} [m^2/s^2]$")
plt.ylim([0,4])
plt.xlim([0,1.5])
plt.ylabel(r'$y [m]$')
#plt.savefig(os.path.join(fig, 'uu_b.eps'))
plt.show()
...
plt.xlim([0,0.7])
plt.ylabel(r'$y [m]$')
#plt.savefig(os.path.join(fig, 'vv_b.eps'))
plt.show()
...
plt.xlim([0,1.0])
plt.ylabel(r'$y [m]$')
#plt.savefig(os.path.join(fig, 'ww_b.eps'))
plt.show()
